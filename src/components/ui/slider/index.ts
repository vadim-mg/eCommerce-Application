...
export default class Slider extends BaseElement<HTMLElement> {
  ...
  constructor(className: string | string[], size: ImageSize, images: Image[], positionControlsPanel: SliderPositionControlsPanel) {
    super({ tag: 'div', class: className });
    this.node.classList.add(classes.slider);
    this.#images = images;
    this.#size = size;
    this.#index = 0;
    const classPositionControl = positionControlsPanel === SliderPositionControlsPanel.INSIDE ? classes.controlsInside : classes.controlsOutside;
  ...
  #createSlider = () => {
    ...
    this.#images.forEach((image) => {
      ...
      const imageLi = new BaseElement<HTMLLIElement>({ tag: 'li', class: classes.imageLi },
        new BaseElement<HTMLImageElement>({ tag: 'img', class: classes.image, src: url, alt: image.label })
      );
  ...
  #createControlsPanel = () => {
    ...
    const controlsPanel = new BaseElement<HTMLElement>({ tag: 'div', class: classes.controlsPanel });

    const arrowLeft = new BaseElement<HTMLElement>({ tag: 'div', class: classes.arrowLeft });
    arrowLeft.node.addEventListener('click', () => this.#rotation(Direction.LEFT));
    arrowLeft.node.innerHTML = arrowLeftSVG;
    const arrowRight = new BaseElement<HTMLElement>({ tag: 'div', class: classes.arrowRight });
    arrowRight.node.addEventListener('click', () => this.#rotation(Direction.RIGHT));
    arrowLeft.node.innerHTML = arrowRightSVG;

    const indicatorsWrapper = new BaseElement<HTMLElement>({ tag: 'div', class: classes.indicators });
    this.#indicators = [];
  ...
  #rotation = (direction: Direction) => {
    if (direction === Direction.LEFT) this.#index -= 1;
    if (direction === Direction.RIGHT) this.#index += 1;
